...
name: video-use
description: >-
  Codex + LibTV 后期视频使用与剪辑 skill。用于整理 LibTV 生成的单镜头素材，制定剪辑策略、生成 EDL、字幕、基础调色、音频衔接、低清预览和成片检查。测试阶段先提出剪辑方案并等待确认；缺少 helper 脚本或依赖时只输出可人工执行的剪辑清单。
---

> 本地 Codex + LibTV 测试说明：
本地测试覆盖规则：不要未经确认直接剪最终版。先基于用户选中的 LibTV 片段输出剪辑策略和低清预览计划；缺少原仓库 helper、转写工具或 ffmpeg 依赖时，停止在 EDL/剪辑清单层，不伪造预览文件。
