...
description: Create a two-stage director-board workflow: first generate a GPT Image 2 storyboard prompt with axis-of-action, blocking, screen direction, eyeline, camera-zone, and start/end staging controls; then generate a Seedance 2.0 video prompt that executes those storyboard images as references. Use when a user has character references, a scene or script, an action concept, or a shot progression idea and wants controlled staging, reference-image generation, storyboard continuity, or a reusable image-to-video pipeline rather than a single fixed prompt. 中文触发包括：导演设定板、轴线、站位、人物调度、参考图生图、故事板参考生视频、图生视频关键帧。
---
...
- ending state
- effect discipline
- axis of action and approved camera side
- character start/end positions, screen direction and eyelines

## Director-Board Geometry Gate

For dialogue, confrontation, chase, fight, entrance/exit, prop handoff, or any multi-character scene, read [references/director-board-geometry.md](references/director-board-geometry.md) before writing either prompt.

Lock scene geometry before choosing panel compositions. The storyboard must make the axis, camera zone, character positions, facing directions, travel directions, eyelines, prop ownership and legal axis-crossing method visible. Stage 2 must treat these as continuity constraints carried by the storyboard reference, not as optional style cues.
...
- environment economy
- escalation logic
- scene axis and allowed camera zone
- per-panel start position, movement path and end position
- screen direction, eyeline target and prop ownership
...
- the established axis, camera side, screen-left/right positions and eyeline vectors
- visible transitions whenever a character or camera legitimately reforms or crosses the axis
...
For scenes governed by spatial continuity, apply the Director-Board Geometry Gate before filling [references/storyboard-template.md](references/storyboard-template.md). Do not generate polished panels until the blocking ledger is internally consistent.
...
- the director board locks axis, camera zone, start/end blocking, screen direction and eyelines when spatial continuity matters
