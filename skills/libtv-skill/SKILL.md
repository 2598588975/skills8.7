...
name: libtv-skill
description: >-
  Codex + LibTV 测试阶段执行器。仅在用户明确要求使用 LibTV/Liblib/LibTV 会话、上传参考图或视频到 LibTV、查询或下载 LibTV 结果，或已确认通过 LibTV 生成单个镜头时使用。测试阶段禁止自动提交完整短剧、批量任务或多项目切换，必须等待用户说“确认生成”。
---

> 本地 Codex + LibTV 测试说明：
本地测试覆盖规则：此 skill 只做 LibTV 执行器。优先复用已有 session；每次最多提交 1 个生图或生视频任务；任何 API 生成前先列出镜头编号、完整提示词、参考图、画幅、时长、分辨率和数量，并等待用户明确回复“确认生成”。
